...
# global prefix). Run a no-op `node` instead of the server's bin so the build
# resolves+caches the package without starting (and hanging on) the server.
...
node --version
npm --version
echo ">>> n8n-mcp pre-installed (global + npx cache at ${NPM_CONFIG_CACHE})"
