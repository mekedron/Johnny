...
`check.sh` = "is gog authed"). MCP tools (trt.36, shipped 2026-06-12) carry the
same shape from a different source: an enabled server's cached tools render
unavailable-with-reason while its last probe failed (docs/MCP.md). Probe failures
hold a skill back as *could-not-verify* — unknown is never
assumed available. The capability POLICY (trt.38, shipped 2026-06-12) composes
*downstream* of this predicate rather than inside it: a policy-denied kind becomes a
`hidden` catalog entry rendered in **neither** prompt block (stronger than
unavailable-with-reason — the canonical least-privilege scenario forbids even
mentioning it), the same gate backstop speaks the policy decline on a forced
attempt, and a `policy_denied` event names the denying layer — see
`docs/CAPABILITY-POLICY.md`. Concretely, per the bead's build list:
...
image, but the router prompt never talks to the sandbox. Since Johnny-wks the
sandbox is **per-workspace** ([WORKSPACES.md](WORKSPACES.md)): assembly resolves
the agent's attached workspace once at dispatch, stamps it into the snapshot, and
every probe/scan below targets THAT workspace's container + skill tree — so two
agents on different workspaces render disjoint catalogs from the same machinery
(the canonical least-privilege scenario: the progress agent's prompt never even
mentions finance kinds):
...
| Configurable capability policies (layered allow/deny + safe-bins) | Johnny-trt.38 | **shipped** (2026-06-12) — `johnny/skills/capability_policy.py` resolution engine (GLOBAL → AGENT → SESSION-MODE → SESSION, deny wins at every merge, `tools_allow` redefines / `tools_also_allow` extends / glob matching incl. `mcp__server__*`; editable safe-bins baseline, removals beat skill grants); `capability_policies` table (migration 0030, one row per scope target) + `/capability-policies` CRUD + `POST /resolve` inspector (allowed/denied + deciding layer); resolved policy rides the trt.41 agent snapshot (catalog transform: policy-denied kinds become `hidden` entries rendered in NEITHER prompt block; gate backstop speaks the policy decline on a forced delegate); worker re-resolves fresh per claimed task — edits bite without restart (live-proven); enforced denials emit `policy_denied` conversation events naming the layer (gate/worker/sandbox_exec surfaces); per-flag bin profiles documented out of scope. See docs/CAPABILITY-POLICY.md |
| Workspaces (per-agent execution environments: container + skill tree + keyring per workspace; dispatch-stamped routing; per-workspace catalogs/installs/accounts) | Johnny-wks.1–6 | **shipped** (2026-06-12) — agents attach to a workspace (`agents.workspace_id`, NULL = default); the trt.22/24 worker and session assembly resolve sandbox URL + skills dir from the snapshot stamp (`resolve_sandbox_url`/`resolve_skills_dir` twins); lazy `johnny-workspace-<id>` containers with idle-TTL sweep; per-workspace gog identity; capstone scenario + structural isolation assertions in docs/WORKSPACES.md |
| MCP connector (config + lazy executor client + `mcp__server__tool`) | Johnny-trt.36 | **shipped** (2026-06-12) — `mcp_servers` table (migration 0031, provider-settings pattern: transport stdio-in-sandbox \| http, encrypted env/headers, include/exclude globs, timeouts) + `/mcp-servers` CRUD + probe (connect → `tools/list` → verdict persisted; failed probe keeps the stale cache so the catalog renders unavailable-with-reason per trt.55); catalog assembly reads the cached view (never connects) and merges internal → skills → mcp; worker chain gains the MCP leg (skills miss → `mcp__` kinds resolve via `McpClientManager`: lazy connect on first reference, config fingerprint reuse, idle-TTL eviction on the sweep, transparent reconnect; configs re-read per execution — no-restart edits); stdio servers spawn INSIDE the skills-sandbox via the new execd `/mcp/*` JSON-RPC line bridge (official `mcp` SDK `ClientSession` drives both transports; reference fixture baked at `/opt/sandbox/mcp_fixture_server.py`); every failure leg settles spoken-form (policy trt.38 globs like `mcp__shady__*` apply unchanged). See docs/MCP.md |
