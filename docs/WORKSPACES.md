...
container instance + one host state directory + the credentials connected
into it. Agents attach to exactly one workspace (`agents.workspace_id`);
everything a session or delegated task does in the sandbox — which skills
exist, which binaries run, *as whom* `gog` talks to Google — happens inside
the attached agent's workspace and nowhere else.
...
uses the same skill install and the same connected account — one auth,
many agents. Isolation is the complement: nothing in a workspace is
reachable from an agent attached to a different one, at any layer
(host paths, container mounts, rendered catalogs, prompt blocks).
...
| --- | --- | --- |
| Container | the always-on compose `skills-sandbox` service | `johnny-workspace-<id>`, **lazily launched** from the same `johnny-skills-sandbox:latest` image on first need (dispatch, worker claim, capabilities/accounts GET, manual Start) |
...
Per-workspace `gog` identity is **one bind + one env var**: the workspace's
...
Connecting an account (`/workspaces`-detail → Accounts, or the agent edit
page) uses gog's remote two-step OAuth: step 1 runs inside the target
container (no callback listener, no published port); the api relays the
browser redirect (`GET /workspaces/accounts/oauth/callback`) into the same
container for the step-2 exchange. One Redis pending record serializes
connects and drives the UI lock. Disconnect and delete affordances remove
only that workspace's identity; the keyring survives idle-TTL restarts and
factory resets (it's a host dir).
...
`/workspaces/{id}` adds container Start/Stop, the per-workspace inventory
(probe-gated when the container is idle, because the capabilities GET
lazily starts it), and the accounts panel. The agent edit page carries the
attachment picker (default preselected; explicit null = back to default).
