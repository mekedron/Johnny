...
Batch-only Johnny STT providers (faster-whisper, Parakeet, ElevenLabs Scribe)
buffer the whole ``audio_iter`` and only emit ``FINAL_TRANSCRIPT``\\ s once it is
exhausted — under LiveKit's continuously-fed ``RecognizeStream`` they would
never emit mid-call. :func:`johnny.agent.adapters.johnny_stt.build_stt_adapter`
therefore wraps exactly these in LiveKit's
:class:`~livekit.agents.stt.StreamAdapter`, which segments the audio with a
Silero VAD and runs the wrapped :meth:`JohnnySTT.recognize` (batch) on each
speech segment. Truly-streaming providers (Deepgram) are driven directly.
...
* classification — streaming names pass through to a bare :class:`JohnnySTT`;
  the three batch names are wrapped in a :class:`StreamAdapter`;
...
class _FakeBatchSTTProvider(STTProvider):
    ...
    def __init__(
        self, *, name: str = "faster-whisper", texts: Sequence[str] | None = None
    ) -> None:
        self._name = name
    ...
    async def transcribe_stream(
        ...
        yield TranscriptEvent(
            text=text, is_final=True, timestamp_ms=0, confidence=0.9
        )
    ...
    *,
    ...
    kwargs: dict[str, Any] = {
        "conn_options": APIConnectOptions(
            max_retry=0, retry_interval=0.0, timeout=5.0
        )
    }
...
def test_streaming_provider_ignores_supplied_vad() -> None:
    adapter = build_stt_adapter(
        _FakeBatchSTTProvider(name="deepgram"), vad=_SilenceSegmentingVAD()
    )
    assert isinstance(adapter, JohnnySTT)
...
async def test_single_utterance_emits_one_final() -> None:
    ...
    adapter = build_stt_adapter(
        provider, vad=_SilenceSegmentingVAD(), language="en-US"
    )
    speech = _speech()
...
async def test_silence_only_input_emits_no_final() -> None:
    provider = _FakeBatchSTTProvider(name="parakeet")
    adapter = build_stt_adapter(provider, vad=_SilenceSegmentingVAD())
...
def test_default_vad_loaded_lazily_when_none(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    adapter = build_stt_adapter(_FakeBatchSTTProvider(name="parakeet"), vad=None)
