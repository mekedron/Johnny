...
Since Johnny-7g5.1 the browser path runs on the in-process LiveKit
...
These tests exercise ``run_browser_pipeline``'s error handling without a
live session: an under-configured payload must surface a clean
``BrowserRunOutcome("failed", …)`` (graceful, never a crash).
"""
...
def _spec(provider_payload: dict | None = None) -> BrowserPipelineSpec:
...
@pytest.mark.asyncio
async def test_underconfigured_payload_fails_gracefully() -> None:
    """A spec with no STT/LLM/TTS rows surfaces a clean failure, not a crash."""
    transport = BrowserAudioTransport()
    stop_event = asyncio.Event()
    outcome = await asyncio.wait_for(
        run_browser_pipeline(transport, _spec({}), stop_event=stop_event),
        timeout=15,
    )
