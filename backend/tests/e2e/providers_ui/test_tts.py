"""TTS provider matrix — exercises the public API path the UI calls."""
...
@pytest.mark.parametrize(
    "plan",
    plans_by_kind(ProviderKind.TTS),
    ids=lambda p: p.plan_id,
)
def test_tts_provider_lifecycle(plan: ProviderPlan, johnny_api: JohnnyAPI) -> None:
    """For each TTS backend with a key/asset, walk the full lifecycle."""
